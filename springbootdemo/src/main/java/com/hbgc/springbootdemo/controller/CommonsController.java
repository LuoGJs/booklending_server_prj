...
public class CommonsController extends BaseController {
    ...
    //给注册手机发送验证码
    @GetMapping("mrcode1")
    public Map<String, Object> sendMobileRandomCodeToRege(String mobile) {

        String randomCode = null;
        try {
            if (usersService.queryUsersByMobile(mobile) != null) {
                //说明该手机已经注册
                return Json.fail("该手机已经注册");
            }
    ...
    //给手机登录发送验证码
    @GetMapping("mrcode2")
    public Map<String, Object> sendMobileRandomCodeToLogin(String mobile) {

        String randomCode = null;
        try {
            if (usersService.queryUsersByMobile(mobile) == null) {
                //说明该手机已经注册
                return Json.fail("该手机还未注册");
            }
            randomCode = RandomUtil.randomNumbers(6);
            //把这个验证码保存到session;做app这样是行不通的。
