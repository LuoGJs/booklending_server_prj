...
//            System.out.println("password=" + user.getPassword());
    ...
    @GetMapping("login4")
    public Map<String,Object> loginByMobile(String mobile,String token,String validateCode){

        Users loginUser = usersService.queryUsersByMobile(mobile);
        try {
            if (loginUser == null) {
                //说明该手机还未注册
                return Json.fail("该手机号还没有注册");
            }
            ...
            //服务器需要一种算法，返回给客户端一个token，这个token作用是为了以后进行身份验证。
            //用登录成功的用户生成一个token值。
            String auth_token = TokenUtils.getToken(loginUser.getUsername());
            loginUser.setToken(auth_token);
            return Json.success(loginUser, "登录成功!");
        } catch (Exception ex) {
            ex.printStackTrace();
...
@PatchMapping("update")
    ...
    @GetMapping("login5") //@RequestParam
    public Map<String, Object> loginInMobile(String username,String password) {
